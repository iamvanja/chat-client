...
export function checkIndexAuthorization({ dispatch }) {
    ...
    return (nextState, replace, next) => {
        // If we pass the authentication check, go to chat
...
export function checkChatAuthorization({ dispatch, getState }) {
    ...
    return (nextState, replace, next) => {
        ...
        // is it defined and does it have a token? good, go ahead to chat
        if (client && client.token) {
            return next();
        }

        // not set yet?  Let's try and set it and if so, go ahead to chat
        if (checkAuthorization(dispatch)) {
            return next();
        }
